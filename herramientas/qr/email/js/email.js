...
(() => {
  ...
  function nombreDescarga(ext) {
    return "codigo-qr-correo" + "." + ext;
  }

  const campoDestinatario = $('[data-campo="destinatario"]');
  const campoAsunto = $('[data-campo="asunto"]');
  const campoCuerpo = $('[data-campo="cuerpo"]');
  [campoDestinatario, campoAsunto, campoCuerpo].forEach((el) => el.addEventListener("input", regenerar));

  function actualizar() {
    const destinatario = campoDestinatario.value.trim();
    if (!destinatario) { mostrarVacio(); return; }
    const payload = L.payloadEmail({
      destinatario,
      asunto: campoAsunto.value.trim(),
      cuerpo: campoCuerpo.value.trim(),
    });
    nombreArchivoEl.textContent = destinatario;
    pintar(payload);
  }
