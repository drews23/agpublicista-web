...
(() => {
  ...
  function nombreDescarga(ext) {
    return "codigo-qr-contacto" + "." + ext;
  }

  const campos = {};
  ["nombre", "apellido", "telefono", "email", "empresa", "cargo", "sitio"].forEach((id) => {
    campos[id] = $('[data-campo="' + id + '"]');
    campos[id].addEventListener("input", regenerar);
  });

  function actualizar() {
    const valores = {};
    for (const id in campos) valores[id] = campos[id].value.trim();
    if (!valores.nombre && !valores.apellido && !valores.telefono && !valores.email) {
      mostrarVacio();
      return;
    }
    const payload = L.payloadVCard(valores);
    nombreArchivoEl.textContent = [valores.nombre, valores.apellido].filter(Boolean).join(" ") || "Contacto";
    pintar(payload);
  }
