...
(() => {
  ...
  function nombreDescarga(ext) {
    return "codigo-qr-sms" + "." + ext;
  }

  const campoNumero = $('[data-campo="numero"]');
  const campoMensaje = $('[data-campo="mensaje"]');
  const contador = $("[data-contador]");
  [campoNumero, campoMensaje].forEach((el) => el.addEventListener("input", () => {
  ...
  function actualizar() {
    ...
    if (!numero) { mostrarVacio(); return; }
    const payload = L.payloadSms({ numero, mensaje: campoMensaje.value.trim() });
    nombreArchivoEl.textContent = numero;
    pintar(payload);
  }
